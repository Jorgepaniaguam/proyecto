...
// Definición de estructuras para libros, usuarios y bibliotecarios
struct SLibro {
	string sTitulo;
	string sAutor;
	string sISBN;
...
// Función para llenar los datos de libros, usuarios y bibliotecarios de manera aleatoria
...
// Función para imprimir los datos de un bibliotecario
...
void BuscarLibro(const string& sBusqueda, const SLibro libros[], int nNumLibros) {
	...
	for (int i = 0; i < nNumLibros; ++i) {
		// Buscar coincidencias en el título del libro
		if (libros[i].sTitulo.find(sBusqueda) != string::npos) {
			contador++; // Incrementar el contador de libros encontrados
			cout << endl << "-" << libros[i].sTitulo; // Imprimir el título del libro encontrado
		}
	}
...
// Función principal
int main() {
	// Definición de constantes para el número de libros, usuarios y bibliotecarios
	const int NUM_LIBROS = 5; // Número de libros predefinidos
	const int NUM_USUARIOS = 3; // Número de usuarios predefinidos
	const int NUM_BIBLIOTECARIOS = 3; // Número de bibliotecarios predefinidos
	
	// Inicialización de la semilla para la generación de números aleatorios
	srand(time(nullptr));
	
	// Declaración de arreglos para almacenar libros, usuarios y bibliotecarios
